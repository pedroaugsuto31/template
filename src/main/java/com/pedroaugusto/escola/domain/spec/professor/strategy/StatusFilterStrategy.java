...
public class StatusFilterStrategy implements FilterStrategy<ProfessorFilter, Professor> {
    ...
    public void apply(ProfessorFilter filtro, Root<Professor> root, CriteriaQuery<?> query, CriteriaBuilder builder, List<Predicate> predicates) {
        Optional.ofNullable(filtro.getStatus())
                .ifPresent(status -> predicates.add(builder.equal(root.get("status"), status)));
    }
