...
public class CidadeFilterStrategy implements FilterStrategy<ProfessorFilter, Professor> {
    ...
    public void apply(ProfessorFilter filtro, Root<Professor> root, CriteriaQuery<?> query, CriteriaBuilder builder, List<Predicate> predicates) {
        Optional.ofNullable(filtro.getCidade())
                .map(String::toLowerCase)
                .ifPresent(cidade -> predicates.add(builder.like(builder.lower(root.get("endereco").get("cidade")), "%" + cidade + "%")));
    }
