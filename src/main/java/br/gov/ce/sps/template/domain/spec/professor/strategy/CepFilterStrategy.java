...
public class CepFilterStrategy implements FilterStrategy<ProfessorFilter, Professor> {
    ...
    public void apply(ProfessorFilter filtro, Root<Professor> root, CriteriaQuery<?> query, CriteriaBuilder builder, List<Predicate> predicates) {
        Optional.ofNullable(filtro.getCep())
                .map(String::strip)
                .ifPresent(cep -> predicates.add(builder.equal(root.get("endereco").get("cep"), cep)));
    }
