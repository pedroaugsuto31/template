...
public class SexoFilterStrategy implements FilterStrategy<ProfessorFilter, Professor> {
    ...
    public void apply(ProfessorFilter filtro, Root<Professor> root, CriteriaQuery<?> query, CriteriaBuilder builder, List<Predicate> predicates) {
        Optional.ofNullable(filtro.getSexo())
                .ifPresent(sexoEnum -> predicates.add(builder.equal(root.get("sexo"), sexoEnum)));
    }
